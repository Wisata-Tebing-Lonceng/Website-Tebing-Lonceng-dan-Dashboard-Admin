...
<head>
    ...
    <title>Tebing Lonceng - Storytelling Experience</title>
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        base: '#FBF9F6',       // Cream Base
                        charcoal: '#1a1a1a',   // Dark Charcoal Text
                        sage: '#6b7b62',       // Refined Sage Green
                        clay: '#c5a27d',       // Muted Sand/Gold
                        darkbase: '#141414',   // For dark sections
                    },
                    fontFamily: {
            ...
            <div class="navbar rounded-full px-6 transition-all duration-500 border backdrop-blur-xl" 
                ...
                <div class="navbar-start w-auto lg:w-1/3">
                    ...
                    <div class="dropdown">
                        ...
                        <ul tabindex="0" class="menu menu-sm dropdown-content mt-3 z-[200] p-2 shadow-xl bg-white/97 backdrop-blur-xl rounded-[1.5rem] w-56 border border-black/5 gap-1 font-medium text-charcoal">
            ...
            <div class="w-full max-w-4xl mx-auto px-4 sm:px-6 mt-20 gs-review-form">
                ...
                <?php if (isset($_SESSION['user_id'])): ?>
                    ...
                    <div class="text-center mb-8">
                        <div class="inline-flex items-center gap-2 px-4 py-2 bg-sage/8 rounded-full text-[10px] font-black text-sage tracking-[0.2em] uppercase mb-5 border border-sage/15">
                            <i class="fi fi-rr-pencil"></i> Bagikan Pengalaman Anda
                        </div>
                        <h3 class="text-3xl font-serif text-charcoal mb-2">Tulis Ulasan</h3>
                        <p class="text-charcoal/50 text-sm">Cerita Anda menginspirasi pengunjung berikutnya.</p>
                    </div>

                    <?php if (!empty($_SESSION['auth_error'])): ?>
                        <div class="mb-5 px-4 py-3 bg-red-50 border border-red-100 rounded-2xl text-red-600 text-sm text-center">
                            <?= htmlspecialchars($_SESSION['auth_error']) ?>
                            <?php unset($_SESSION['auth_error']); ?>
                        </div>
                    <?php endif; ?>

                    <form id="review-form" action="actions/user/tambah_review.php" method="POST" class="flex flex-col gap-5">
                        <!-- Rating -->
                        <div>
                            <label class="text-[10px] uppercase font-black tracking-widest text-charcoal/40 mb-3 block">Rating</label>
                            <div class="rating rating-lg gap-1">
                                <input type="radio" name="rating" value="1" class="mask mask-star-2 bg-clay/70 hover:bg-clay cursor-pointer" />
                                <input type="radio" name="rating" value="2" class="mask mask-star-2 bg-clay/70 hover:bg-clay cursor-pointer" />
                                <input type="radio" name="rating" value="3" class="mask mask-star-2 bg-clay/70 hover:bg-clay cursor-pointer" />
                                <input type="radio" name="rating" value="4" class="mask mask-star-2 bg-clay/70 hover:bg-clay cursor-pointer" />
                                <input type="radio" name="rating" value="5" class="mask mask-star-2 bg-clay/70 hover:bg-clay cursor-pointer" checked />
                            </div>
                        </div>
                        <!-- Textarea -->
                        <div>
                            <label class="text-[10px] uppercase font-black tracking-widest text-charcoal/40 mb-2 block">Ulasan Anda</label>
                            <textarea name="kesan" class="textarea w-full bg-[#fbf9f6] border border-black/8 rounded-2xl text-charcoal placeholder:text-charcoal/30 focus:border-sage/60 focus:outline-none transition-colors resize-none text-base font-serif italic leading-relaxed min-h-[160px]" placeholder="&ldquo;Ceritakan pengalaman Anda di Tebing Lonceng...&rdquo;" required minlength="20"></textarea>
                        </div>
                        <!-- Error message -->
                ...
                <?php else: ?>
                    ...
                    <div class="text-center mb-8">
                        <div class="inline-flex items-center gap-2 px-4 py-2 bg-clay/10 rounded-full text-[10px] font-black text-clay tracking-[0.2em] uppercase mb-5 border border-clay/20">
                            <i class="fi fi-rr-lock"></i> Login untuk Berkomentar
                        </div>
                        <h3 class="text-3xl font-serif text-charcoal mb-2">Tulis Ulasan</h3>
                    ...
                    <div class="flex flex-col gap-6">
                        <div>
                            ...
                            <div
                                ...
                                @click.stop
                            >
                                <div class="bg-base rounded-[1.5rem] shadow-2xl border border-black/8 overflow-hidden">
                                    <calendar-date
                                        class="cally"
...
<script>
    ...
    let smootherInstance = null; // Global reference for ScrollSmoother
    ...
    createApp({
        setup() {
            ...
            const scrollToSection = (targetId) => {
                if (smootherInstance) {
                    smootherInstance.scrollTo(`#${targetId}`, true, "top top");
                }
            ...
            onMounted(() => {
                ...
                // Create the smooth scroller FIRST before any ScrollTriggers
                ...
                // Parallax Hero Video via ScrollSmoother data-speed effects
